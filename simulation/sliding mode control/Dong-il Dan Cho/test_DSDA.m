...
%% plot
plot_ctr(t,Xd,X,u_act)        % plot control performance
...
function simOut = sim_DSVC_DDC(time,Pssd,DSVC_DDC_parm)
    ...
    for k = 2:lenT
        ...
        %% Auxiliary state
        Delta_u_pre = dz(u_pre,ulim);
        z = a*z_pre - K*B*Delta_u_pre;
        ...
        u = -dest + (K*B)\(K*Xd_next - K*A*X - a*z + q*s - Eta*sat(s,phi));
